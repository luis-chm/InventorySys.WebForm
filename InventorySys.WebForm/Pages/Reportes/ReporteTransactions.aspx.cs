...
{
    public partial class ReporteTransactions : System.Web.UI.Page
    {
        ...
        protected void btnReporteByDate_Click(object sender, EventArgs e)
        {
            ...
            MemoryStream memoria = reportsBL.ReporteMaterialTransactionsByDate(txtFechaInicio.Text, txtFechaFin.Text);

            var nombreExcel = $"Reporte Transacciones {DateTime.Now:yyyyMMddHHmmss}.xlsx";
            Response.Clear();
        ...
        {
            MemoryStream memoria = reportsBL.ReporteMaterialTransactionsGeneral();

            var nombreExcel = $"Reporte General Transacciones {DateTime.Now:yyyyMMddHHmmss}.xlsx";
            Response.Clear();
            Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
            Response.AddHeader("content-disposition", $"attachment; filename={nombreExcel}");
            Response.BinaryWrite(memoria.ToArray());
